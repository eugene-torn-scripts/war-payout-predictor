...
FREF = 40.0   # fighter reference
MREF = 50.0   # opponent-member reference
...
for line in open(os.path.join(DATA, "reports.jsonl")):
    ...
    for i, f in enumerate(facs):
        ...
        base = {"rank": f["rank_before"], "won": won, "enl": f["enlisted"], "v": v, "y": math.log(v),
                ...
                "p": n10 / f["enlisted"], "loe": math.log(opp["enlisted"]) if opp["enlisted"]>0 else 0.0}
        srows.append(base)
        if base["p"] > 0:
...
# SCORE model: + own & opp fighters + opponent total members
bs = ols(srows, [lambda r: r["ls"], lambda r: r["ls"] ** 2, lambda r: r["ln"],
                 lambda r: r["lon"], lambda r: r["loe"]],
         ["a", "b", "c", "d", "e"], dummies, BASE)
A, B, C, D, E = bs["a"], bs["b"], bs["c"], bs["d"], bs["e"]
ls_star = -A / (2 * B)
...
def score_predlog(r):
    ...
    return (bse + math.log(2.0) * r["won"] + math.log(1 + 0.01 * (r["enl"] - 10))
            + A * r["ls"] + B * r["ls"] ** 2 + C * r["ln"] + D * r["lon"] + E * r["loe"])

ds = diag(srows, score_predlog)
# anchored base: value = baseD · 2^won · sizeF · partD · (own/FREF)^C · (oppF/FREF)^D · (oppEnl/MREF)^E
score_base = {rk: math.exp(bs["intercept"] + (0 if rk == BASE else bs["rank:" + rk]))
              * M / 3 * (FREF ** C) * (FREF ** D) * (MREF ** E) for rk in ranks}
...
print("coeffs: ownFighters^%.3f  oppFighters^%.3f  oppMembers^%.3f  (Fref=%d Mref=%d)" % (C, D, E, FREF, MREF))
...
model = {
    ...
    "size_per_member": 0.01, "fighter_ref": FREF, "opp_member_ref": MREF,
    "cache_prices": {str(k2): v for k2, v in prices.items()},
    "score_model": {"share_a": A, "share_b": B, "part_max": M, "own_c": C, "opp_d": D, "opp_e": E,
...
def cs(rank, won, enl, share, own, opp, oppenl):
    ls = math.log(share)
    partD = 3 * (M if ls >= ls_star else math.exp(A * ls + B * ls ** 2)) / M
    return (score_base[rank] * (2.0 ** won) * (1 + 0.01 * (enl - 10)) * partD
            * (own / FREF) ** C * (opp / FREF) ** D * (oppenl / MREF) ** E)

print("ADHD (Gold I,loss,92,share .054, 3 vs 62 fighters, opp 100): $%.0fm (actual $115m)"
      % (cs("Gold I", 0, 92, 726 / 13454, 3, 62, 100) / 1e6))
print("L&L  (Gold,win,100,share .946, 62 vs 3 fighters, opp 92)  : $%.2fb (actual $2.57b)"
      % (cs("Gold", 1, 100, 12728 / 13454, 62, 3, 92) / 1e9))
